  ...
  "node_modules/@solana-commerce/connector/dist/index.mjs"() {
    ...
    ConnectorClient = class {
      ...
      async selectAccount(address) {
        ...
        let target = this.state.accounts.find((acc) => acc.address === address)?.raw ?? null;
        if (!target) {
          try {
            ...
            if (feature) {
              ...
              target = accounts.find((acc) => acc.address === address)?.raw ?? res.accounts[0] ?? null;
...
var require_wallet = __commonJS({
  "static/js/wallet.js"() {
    ...
    async function postJson(url, dados = {}) {
      ...
      if (!resposta.ok) {
        throw new Error(
          resultado.erro || "Ocorreu um erro."
        );
      }
      return resultado;
    }
    async function conectarCarteira(area) {
      let state = connector.getConnectorState();
      const carteiras = state.wallets.filter(
        (wallet) => wallet.connectable
      );
      if (!carteiras.length) {
        throw new Error(
          "Nenhuma carteira Solana compat\xEDvel foi encontrada. Instale ou desbloqueie a Phantom."
        );
      }
      const phantom = carteiras.find(
        (wallet) => wallet.name.toLowerCase().includes("phantom")
      );
      const carteiraEscolhida = phantom || carteiras[0];
      await connector.select(
        carteiraEscolhida.name
      );
      state = connector.getConnectorState();
      const endereco = state.selectedAccount;
      if (!endereco) {
        throw new Error(
          "N\xE3o foi poss\xEDvel obter o endere\xE7o da carteira."
        );
      }
      await postJson(
    ...
    async function verificarCarteira(area) {
      ...
      const accountInfo = state.accounts.find(
        (conta) => conta.address === endereco
      );
      if (!accountInfo) {
        throw new Error(
          "Conta Solana n\xE3o encontrada."
        );
      }
      const wallet = state.selectedWallet;
      const recursoAssinatura = wallet.features["solana:signMessage"];
      if (!recursoAssinatura) {
        throw new Error(
          "Esta carteira n\xE3o suporta assinatura de mensagens."
        );
      }
      const mensagemBytes = new TextEncoder().encode(
        desafio.mensagem
      );
      const resultados = await recursoAssinatura.signMessage(
        {
      ...
      if (!resultado) {
        throw new Error(
          "A carteira n\xE3o retornou uma assinatura."
        );
      }
      await postJson(
    ...
    document.addEventListener(
      ...
      () => {
        ...
        const botaoConectar = document.querySelector(
          "#connect-solana-wallet"
        );
        const botaoVerificar = document.querySelector(
          "#verify-solana-wallet"
        );
        const botaoDesconectar = document.querySelector(
        ...
        if (botaoConectar) {
          botaoConectar.addEventListener(
            ...
            async () => {
              ...
              botaoConectar.textContent = "Conectando...";
              try {
                ...
                botaoConectar.textContent = "Conectar carteira Solana";
              }
            }
          );
        }
        if (botaoVerificar) {
          botaoVerificar.addEventListener(
            ...
            async () => {
              ...
              botaoVerificar.textContent = "Aguardando assinatura...";
              try {
                ...
                botaoVerificar.textContent = "Verificar carteira";
              }
            }
          );
        }
